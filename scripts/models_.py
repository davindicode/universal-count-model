...
def hyper_params(basis_mode="ew"):
    ...
    if basis_mode == "ew":  # element-wise
        ...
        def mix(x):
            N = x.shape[-1]
            out = torch.empty((*x.shape[:-1], N * (N - 1) // 2), dtype=x.dtype).to(
                x.device
            )
            k = 0
            for n in range(1, N):
...
class net(nn.Module):
    def __init__(self, C, basis, max_count, channels, shared_W=False):
        ...
        mnet = utils.pytorch.Parallel_MLP(
            [],
            expand_C,
            (max_count + 1),
            channels,
            shared_W=shared_W,
            nonlin=utils.pytorch.Siren(),
            out=None,
        )
...
def kernel_used(mode, behav_tuple, num_induc, outdims, var=1.0):
    hd_t, w_t, s_t, x_t, y_t, time_t = behav_tuple
    ...
    l = 10.0 * np.ones(outdims)
    l_w = w_t.std() * np.ones(outdims)
    l_ang = 5.0 * np.ones(outdims)
    l_s = 10.0 * np.ones(outdims)
    v = var * np.ones(outdims)
    l_time = time_t.max() / 2.0 * np.ones(outdims)
    l_one = np.ones(outdims)

    if mode == "hd":
        ind_list = [np.linspace(0, 2 * np.pi, num_induc + 1)[:-1]]
        kernel_tuples = [("variance", v), ("RBF", "torus", np.array([l_ang]))]

    elif mode == "w":
        ind_list = [np.linspace(-w_t.std(), w_t.std(), num_induc)]
        kernel_tuples = [("variance", v), ("RBF", "euclid", np.array([l_w]))]
        ...
        kernel_tuples = [
            ...
            (
                "RBF",
                "euclid",
                np.array([l_w, l_s, l, l, l_time, l_one, l_one, l_one, l_one]),
            ),
        ]
        ...
        l = 1.0 * np.ones((1, outdims))
        kernel_tuples = [("variance", v), ("RBF", "euclid", l)]

    elif mode == "R2":
        ind_list = [np.linspace(-1, 1, num_induc), np.random.randn(num_induc)]
        l = 1.0 * np.ones((2, outdims))
        kernel_tuples = [("variance", v), ("RBF", "euclid", l)]
...
def get_VI_blocks(mode, behav_tuple):
    ...
    if mode == "hd":
        ...
        VI_tuples = [(None, None, None, 1)] * (len(covariates) - 1) + [
            (
                ["RW", (np.array([4.0] * 2), np.array([1.0] * 2), True, False)],
                "Normal",
                "euclid",
                2,
            )
        ...
        VI_tuples = [(None, None, None, 1)] * (len(covariates) - 1) + [
            (
                ["RW", (np.array([4.0] * 3), np.array([1.0] * 3), True, False)],
                "Normal",
                "euclid",
                3,
            )
        ...
        VI_tuples = [(None, None, None, 1)] * (len(covariates) - 1) + [
            (
                ["RW", (np.array([4.0] * 4), np.array([1.0] * 4), True, False)],
                "Normal",
                "euclid",
                4,
            )
        ...
        VI_tuples = [
            (["RW", (0.0, 4.0, True, True)], "Normal", "torus", 1),
            (["RW", (4.0, 1.0, True, False)], "Normal", "euclid", 1),
        ]
        ang_dims = 1
        ...
        VI_tuples = [
            (
                ["RW", (np.array([4.0, 4.0]), np.array([1.0, 1.0]), True, False)],
                "Normal",
                "euclid",
                2,
            )
...
def GP_params(
    ...
    gp_mapping = mdl.nonparametrics.Gaussian_process(
        inpd,
        neurons,
        kernel_tuples,
        inv_link=inv_link,
...
def likelihood_params(
    ...
    if ll_mode == "IBP":
        ...
        alpha = 0.1 * np.ones(inner_dims)
        likelihood = mdl.likelihoods.ZI_Poisson(
            tbin, inner_dims, inv_link, alpha, dispersion_mapping=gp_lvms
        )
        # inv_link_hetero = lambda x: torch.sigmoid(x)/tbin

    elif ll_mode == "NB" or ll_mode == "hNB":
        r_inv = 10.0 * np.ones(inner_dims)
        likelihood = mdl.likelihoods.Negative_binomial(
            tbin, inner_dims, inv_link, r_inv, dispersion_mapping=gp_lvms
        )

    elif ll_mode == "CMP" or ll_mode == "hCMP":
        log_nu = np.zeros(inner_dims)
        likelihood = mdl.likelihoods.COM_Poisson(
            tbin, inner_dims, inv_link, log_nu, J=J, dispersion_mapping=gp_lvms
        )

    elif ll_mode == "IG":  # renewal process
        shape = np.ones(inner_dims)
        likelihood = mdl.likelihoods.Gamma(
            tbin, inner_dims, inv_link, shape, allow_duplicate=False
        )

    elif ll_mode == "IIG":  # renewal process
        mu_t = np.ones(inner_dims)
        likelihood = mdl.likelihoods.invGaussian(
            tbin, inner_dims, inv_link, mu_t, allow_duplicate=False
        )

    elif ll_mode == "LN":  # renewal process
        sigma_t = np.ones(inner_dims)
        likelihood = mdl.likelihoods.logNormal(
            tbin, inner_dims, inv_link, sigma_t, allow_duplicate=False
        )

    elif ll_mode == "U":
        likelihood = mdl.likelihoods.Universal(
            inner_dims // C, C, inv_link, cutoff, mapping_net
        )
    else:
