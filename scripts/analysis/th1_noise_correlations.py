...
binning = 40
rcov, neurons, tbin, resamples, rc_t, region_edge = HDC.get_dataset(
    session_id, phase, binning, "../scripts/data"
)
max_count = int(rc_t.max())
x_counts = torch.arange(max_count + 1)
...
for em, mode in enumerate(Ms):
    for cvdata in model_utils.get_cv_sets(
        mode, kcvs, batch_size, rc_t, resamples, rcov
    ):
...
mz1_mean = avg.mean(0)
mz1_ff = ff.mean(0)
z1_mean_tf = (mz1_mean.max(dim=-1)[0] - mz1_mean.min(dim=-1)[0]) / (
...
mz2_mean = avg.mean(0)
mz2_ff = ff.mean(0)
z2_mean_tf = (mz2_mean.max(dim=-1)[0] - mz2_mean.min(dim=-1)[0]) / (
...
if z_d == 1:  ### latent ###
    ...
    steps = 100
    covariates_z = np.linspace(-0.2, 0.2, steps)
    for en, n in enumerate(pick_neuron):
        ...
        P_mc = model_utils.compute_P(full_model, covariates, [n], MC=1000).cpu()[
            :, 0, ...
        ]
