...
def variability_stats():
    ...
    for q in Qq_bn:
        for qq in q:
            T_DS, T_KS, sign_DS, sign_KS, p_DS, p_KS = utils.stats.KS_statistics(
...
def tunings():
    ...
    # TI
    hd_mean_tf = (mhd_mean.max(dim=-1)[0] - mhd_mean.min(dim=-1)[0]) / (
    ...
    mw_mean = avg.mean(0)
    mw_ff = ff.mean(0)
    w_mean_tf = (mw_mean.max(dim=-1)[0] - mw_mean.min(dim=-1)[0]) / (
        mw_mean.max(dim=-1)[0] + mw_mean.min(dim=-1)[0]
    )
    w_ff_tf = (mw_ff.max(dim=-1)[0] - mw_ff.min(dim=-1)[0]) / (
    ...
    ms_mean = avg.mean(0)
    ms_ff = ff.mean(0)
    s_mean_tf = (ms_mean.max(dim=-1)[0] - ms_mean.min(dim=-1)[0]) / (
        ms_ff.max(dim=-1)[0] + ms_ff.min(dim=-1)[0]
    )
    s_ff_tf = (ms_ff.max(dim=-1)[0] - ms_ff.min(dim=-1)[0]) / (
        ms_ff.max(dim=-1)[0] + ms_ff.min(dim=-1)[0]
    )
    ...
    mt_mean = avg.mean(0)
    mt_ff = ff.mean(0)
    t_mean_tf = (mt_mean.max(dim=-1)[0] - mt_mean.min(dim=-1)[0]) / (
        mt_ff.max(dim=-1)[0] + mt_ff.min(dim=-1)[0]
    )
    t_ff_tf = (mt_ff.max(dim=-1)[0] - mt_ff.min(dim=-1)[0]) / (
        mt_ff.max(dim=-1)[0] + mt_ff.min(dim=-1)[0]
    )
    ...
    mpos_ff = ff.mean(0)
    pos_mean_tf = (mpos_mean.max(dim=-1)[0] - mpos_mean.min(dim=-1)[0]) / (
    ...
    steps = 100
    covariates_s = np.linspace(0, 30.0, steps)
    for en, n in enumerate(pick_neuron):
        covariates = [
            pref_hdw[en, len(w_arr) // 2] * np.ones(steps),
            0.0 * np.ones(steps),
